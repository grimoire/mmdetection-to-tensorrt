...
def get_conda_packages(run_lambda):
    if get_platform() == 'win32':
        ...
        grep_cmd = r'{} /R "torch numpy cudatoolkit soumith mkl magma mypy"'\
            .format(findstr_cmd)
    else:
        grep_cmd = r'grep "torch\|mmcv\|mmdet\|tensorrt\|torch2trt"'
...
def get_pip_packages(run_lambda):
    ...
    def run_with_pip(pip):
        if get_platform() == 'win32':
            ...
            grep_cmd = r'grep "torch\|mmcv\|mmdet\|tensorrt\|torch2trt"'
        return run_and_read_all(run_lambda,
                                pip + ' list --format=freeze | ' + grep_cmd)
...
def get_env_info():
    ...
    if TORCH_AVAILABLE:
        ...
        version_str = debug_mode_str = cuda_available_str\
    ...
    if TORCH_AVAILABLE and hasattr(torch, 'utils') and hasattr(
        ...
        if sys.platform == 'linux' and os.path.exists(minidump_dir):
            ...
            creation_time = datetime.datetime.fromtimestamp(ctime).strftime(
                '%Y-%m-%d %H:%M:%S')
            msg = '\n*** Detected a minidump at {}'\
                ' created on {}, '.format(latest, creation_time) +\
                'if this is related to your bug '\
                'please include it when you file a report ***'
