...
class SABLHeadWarper(BBoxHeadWarper):
    ...
    def regress_by_class(self, rois, label, bbox_pred, img_shape):
        ...
        if rois.size(1) == 4:
            new_rois, _  = self.bbox_coder.decode(rois.unsqueeze(0), [bb.unsqueeze(0) for bb in bbox_pred], max_shape=img_shape)
        else:
            bboxes, _ = self.bbox_coder.decode(rois[:,1:].unsqueeze(0), [bb.unsqueeze(0) for bb in bbox_pred], max_shape=img_shape)
    ...
    def get_bboxes(self,
        ...
        if rois.size(1) == 4:
            bboxes, confids = self.bbox_coder.decode(rois.unsqueeze(0), [bb.unsqueeze(0) for bb in bbox_pred], max_shape=img_shape)
        else:
            bboxes, confids = self.bbox_coder.decode(rois[:,1:].unsqueeze(0), [bb.unsqueeze(0) for bb in bbox_pred], max_shape=img_shape)
        ...
        num_detections, det_boxes, det_scores, det_classes = self.rcnn_nms(scores, bboxes, num_bboxes, cfg.max_per_img)
