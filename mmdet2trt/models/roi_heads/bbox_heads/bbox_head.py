...
class BBoxHeadWraper(nn.Module):
    ...
    def regress_by_class(self, rois, label, bbox_pred, img_shape):
        ...
        if rois.size(1) == 4:
            new_rois = self.bbox_coder.decode(rois.unsqueeze(0), bbox_pred.unsqueeze(0), max_shape=img_shape)
        else:
            new_rois = self.bbox_coder.decode(rois[:,1:].unsqueeze(0), bbox_pred.unsqueeze(0), max_shape=img_shape)
