...
@register_warper("mmdet.models.GARPNHead")
class GARPNHeadWarper(GuidedAnchorHeadWarper):
    ...
    def forward(self, feat, x):
        ...
        for idx in range(len(cls_scores)):
            ...
            if self.use_sigmoid_cls:
                ...
                rpn_cls_score = rpn_cls_score.reshape(rpn_cls_score.shape[0], -1, 2)
                scores = rpn_cls_score.softmax(dim=2)[:, :, :-1]
            scores = scores*mask

            rpn_bbox_pred = rpn_bbox_pred.permute(0, 2, 3, 1).reshape(rpn_bbox_pred.size(0), -1, 4)
            if nms_pre > 0:
                ...
                bbox_pred = mm2trt_util.pad_with_value(rpn_bbox_pred, 1, nms_pre)
