...
class SABLRetinaHeadWarper(nn.Module):
    ...
    def forward(self, feat, x):
        ...
        mlvl_anchors = self.square_anchor_generator(cls_scores, device = cls_scores[0].device)
        ...
        for cls_score, bbox_cls_pred, bbox_reg_pred, anchors in zip(
            ...
            cls_score = cls_score.permute(0, 2, 3, 1).reshape(batch_size,
                -1, module.cls_out_channels)
            ...
            bbox_cls_pred = bbox_cls_pred.permute(0, 2, 3, 1).reshape(batch_size, -1, self.side_num * 4)
            bbox_reg_pred = bbox_reg_pred.permute(0, 2, 3, 1).reshape(batch_size, -1, self.side_num * 4)
            ...
            if nms_pre > 0:
                ...
                bbox_cls_pred=mm2trt_util.pad_with_value(bbox_cls_pred, 1, nms_pre)
                bbox_reg_pred=mm2trt_util.pad_with_value(bbox_reg_pred, 1, nms_pre)
                ...
                bbox_cls_pred = mm2trt_util.gather_topk(bbox_cls_pred, 1, topk_inds)
                bbox_reg_pred = mm2trt_util.gather_topk(bbox_reg_pred, 1, topk_inds)
