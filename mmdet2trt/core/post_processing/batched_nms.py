...
class BatchedNMS(nn.Module):
    ...
    def forward(self, scores, bboxes, topK, keepTopK):
        ...
        for batch_idx in range(scores.shape[0]):
            ...
            if num_nms < keepTopK:
                nmsed_bbox = torch.nn.functional.pad(nmsed_bbox, pad=(
                    0, 0, 0, keepTopK - num_nms), mode='constant', value=0)
                nmsed_score = torch.nn.functional.pad(nmsed_score, pad=(
                    0, keepTopK - num_nms), mode='constant', value=0)
                nmsed_class = torch.nn.functional.pad(nmsed_class, pad=(
                    0, keepTopK - num_nms), mode='constant', value=-1)
            else:
